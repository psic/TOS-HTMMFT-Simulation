...
import com.htmmft.video.ViewerClientLogMain;

import soccer.client.Controller;
...
public class Field 	extends Arena {
	 ...
	 */
	public Field(ViewerClientMain aSoccerMaster) {
	...
	/**
	 * Draw user controlled ball pass (a blinking vector)
	 */
	private void drawUserBallPass(Graphics2D g2) {
		if (Controller.getKickCommandCount() >= 0
				&& Controller.getKickCommandCount() <= Controller.KICK_COMMAND_DURATION) {
			if (world.getViewData().time % 2 == 0) {
				g2.setColor( Color.BLUE );
				drawVector(g2, world.getBall(), Controller.getKickEndPoint(), false);
			}
		}
	}
