...
import com.htmmft.team.HTMMFTSoccerTeamMain;
...
import soccer.common.TOS_AgentConstants;
...
public class HTMMFTAI  {
	...
	// these objects are used for the communication with the server
		 ...
		 */
		public HTMMFTAI(Transceiver transceiver, char side, int id, 
		        ...
		        if (receivedPacket != null) {	        	
		        	...
		        	if (receivedPacket.packetType == Packet.BYE) {
		        		...
		        		if (byeData.actionType == ByeData.TERMINATE) {
		        			if (!HTMMFTSoccerTeamMain.isTerminated) {
		        				...
		        				HTMMFTSoccerTeamMain.isTerminated = true;	
...
//						lastSoundPlayedMS = System.currentTimeMillis(); 
		...
		private boolean shouldIHoldBall() {
			...
			if (!HTMMFTSoccerTeamMain.isBallHoldingAllowed)
		...
		private BallPassData calcBestPassInSector(MyPlayer myplayer, 
			...
			if (!HTMMFTSoccerTeamMain.isBallPassingAllowed)
		...
		private boolean shouldIdribbleSlow() {
			if (!HTMMFTSoccerTeamMain.isSlowBallDribblingAllowed)
		...
		private void sendKickPacket(double direction, double force) throws IOException {
			...
			Packet commandPacket = new Packet( 
										Packet.KICK, 
										aKickData, 
										HTMMFTSoccerTeamMain.address, 
										HTMMFTSoccerTeamMain.port );
