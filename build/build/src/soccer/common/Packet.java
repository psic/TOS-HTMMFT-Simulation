...
/**
 * This class defines the specialized UDP soccer packet between server 
 * and client.
 * Constructors have been added as the project 
 * functionality was being enhanced.
 *
 * @author Yu Zhang 
 * 		Vadim Kyrylov (since 2006)
 */
...
public class Packet {
  ...
  /** 
   * UDP packet address in real mode:
   * the receipient's address if the packet is sent;
   * the sender's address if the packet is received.
   */
  public InetAddress address;

  /**
   * UDP packet port number in real mode
   * the receipient's port number if the packet is sent;
   * the sender's port number if the packet is received.
   */
  public int port;
  
  /**
   * Packet sender ID in the debug mode;
   * not used by the receiver in the real mode. 
   * Coding conventions:
   * 	0 			the server
   * 	-1..-11 	players in left team 
   * 	 1..11  	players in right team 
   * 	100..110	viewer clients 
   */
  public int senderIDdebug;
  
  /**
   * Constructs a default empty packet for receiving and 
   * parsing data from a UDP socket.
   */
  public Packet()  {
    this.packetType = '?';
  ...
  /**
   * Constructs a clone of given packet
   */
  public Packet(Packet p) {
    ...
    this.port = p.port;
    this.senderIDdebug = p.senderIDdebug;
  } 

  /**
   * Constructs a packet for a particular packet type to be 
   * sent over a UDP socket. The network address is set to 
   * localhost, and port number is set to 7777.
   *
  ...
  /**
   * Constructs a packet for a particular packet 
   * type to be sent over a UDP socket.
   *
   * @param type         the packet type
   ...
   * It checks the first key word to see which packet it is and 
   * proceeds to read the Data content.
   * 
   ...
   * data.
   * 
   * @param     s           the input string which contains data
   * @exception IOException is thrown if things go really bad like 
   * 						wrong data format
   */
  public final void readPacket(String s) throws IOException {
    ...
    // get sender ID for the debug mode 
    senderIDdebug = Integer.parseInt(st.nextToken());
  ...
  //---------------------------------------------------------------------------
  /**
   * Convert Packet object to String for debug print
   */
