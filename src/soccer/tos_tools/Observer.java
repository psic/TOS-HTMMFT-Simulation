...
public class Observer extends Thread {
	...
	private boolean getDataFromServer( Packet packet ) 
	{
		...
		try { 
			...
			if (packet.packetType == Packet.VIEW) {
				...
				handleViewPacket(packet);
				
				
			} else if (packet.packetType == Packet.REFEREE) {
				
				goOn = handleRefereePacket(packet);
	...
	protected void handleViewPacket(Packet packet) {
		...
		// collect statistics
		if (view.ball.getPosition().getX() > 0)
			ball_in_right_side++;
		if (view.ball.getPosition().getX() < 0)
			ball_in_left_side++;
	...
	protected boolean handleRefereePacket(Packet packet){
		boolean goOn = true;
		referee = (RefereeData) packet.data;
		if (referee.period == RefereeData.GAME_OVER)
			goOn = false;				
