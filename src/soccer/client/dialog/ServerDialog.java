...
public class ServerDialog extends JPanel implements ActionListener {
	...
	private String command 
		= "SoccerServer.bat";	
	//= "java -cp soccer.jar soccer.server.SoccerServer";	// TODO
	...
	public ServerDialog(DialogManager mgr, ViewerClientMain soccerMaster) {
	...
	private void setupServerPanel() {
		...
		portField.setEnabled(false);		// TODO
		...
		propertyField.setEnabled(false);		// TODO
		...
		othersField.setEnabled(false);		// TODO
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("On")) {
			if (outputUpdater != null)
				if (!outputUpdater.isOK())
					ViewerClientMain.serverProcess = null;
			if (ViewerClientMain.serverProcess != null) {
			...
			try {
				ViewerClientMain.serverProcess =
					ViewerClientMain.runtime.exec(
						command + " " + port + " " + property + " " + others);
				ps = new PrintStream(ViewerClientMain.serverProcess.getOutputStream());
				ds = new DataInputStream(ViewerClientMain.serverProcess.getInputStream());
				outputUpdater = new OutputUpdater(outputArea, ds, 50);
				outputUpdater.start();
			...
			if (ViewerClientMain.serverProcess != null) {
				try {
					outputArea.setText("");
					ViewerClientMain.serverProcess.destroy();
					ViewerClientMain.serverProcess = null;
					outputUpdater.setOK(false);
	...
	public void display() {
		if (m_dialog == null) {
			m_dialog = new JDialog((Frame) null, "Start Local Server", true);
